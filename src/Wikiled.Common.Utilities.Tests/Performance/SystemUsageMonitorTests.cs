...
namespace Wikiled.Common.Utilities.Tests.Performance
{
    ...
    public class SystemUsageMonitorTests
    {
        ...
        {
            ClassicAssert.Throws<ArgumentOutOfRangeException>(() => instance.Start(TimeSpan.FromMilliseconds(1), TimeSpan.FromMinutes(2)));
            ClassicAssert.Throws<ArgumentOutOfRangeException>(() => instance.Start(TimeSpan.FromMinutes(1), TimeSpan.FromMinutes(1)));

            instance.Start(TimeSpan.FromMinutes(1), TimeSpan.FromMinutes(2));
            ClassicAssert.Throws<Exception>(() => instance.Start(TimeSpan.FromMinutes(1), TimeSpan.FromMinutes(2)));
        }
        ...
        public void Start()
        {
            ...
            ClassicAssert.AreEqual(1, times);
        }

        [Test]
        public void Construct()
        {
            ClassicAssert.Throws<ArgumentNullException>(() => new SystemUsageMonitor(null, mockSystemUsageBucket.Object, scheduler));
            ClassicAssert.Throws<ArgumentNullException>(() => new SystemUsageMonitor(mockSystemUsageCollector.Object, null, scheduler));
            ClassicAssert.Throws<ArgumentNullException>(() => new SystemUsageMonitor(mockSystemUsageCollector.Object, mockSystemUsageBucket.Object, null));
